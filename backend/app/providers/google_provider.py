...
class GoogleProvider(BaseProvider):
    async def call_model(self, request: dict[str, Any]) -> dict[str, Any]:
        model = request.get("model", "google/gemini-pro")
        messages = request.get("messages", [])

        prompt_text = " ".join(m.get("content", "") for m in messages)
        base_prompt_tokens = max(len(prompt_text.split()) * 4, random.randint(200, 850))

        prompt_tokens = int(base_prompt_tokens * random.uniform(0.9, 1.3))
        completion_tokens = random.randint(120, 1300)
